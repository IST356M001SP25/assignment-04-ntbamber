...
def get_column_names(df : pd.DataFrame) -> list[str]:
    ...
    '''
    return df.columns.tolist() #retrives columns names and converts to a list
...
def get_columns_of_type(df : pd.DataFrame, numpy_type: any) -> list[str]:
    ...
    '''
    return df.select_dtypes(include=numpy_type).columns.tolist() #selects columns that contain numpy_type and converts to a list
...
def get_unique_values(df : pd.DataFrame, column_name: str) -> list:
    ...
    '''
    return df[column_name].unique().tolist() #finds unique values in the input column and converts to a list
...
def get_file_extension(file_path : str) -> str:
    ...
    '''
    return file_path.split('.')[-1] #splits the file path on . and returns the last element(the file extension)
...
def load_file(file_path: str, ext: str) -> pd.DataFrame:
    ...
    '''
    if ext == 'csv': #checks if csv
        return pd.read_csv(file_path) #reads csv
    elif ext == 'xlsx': #checks if xlsx
        return pd.read_excel(file_path) #reads xlsx
    elif ext == 'json': #checks if json
        return pd.read_json(file_path) #reads json
